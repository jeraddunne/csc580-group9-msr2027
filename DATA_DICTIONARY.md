...
| `rq3_threshold_sweep.csv` | one row per similarity threshold (0.3 to 0.8) | `threshold`, `lineage_pairs`, `families`, `differing_pairs`, `families_with_differences`, `ordered_pairs`, `ordered_newer_adds`, `ordered_newer_drops`, `ordered_pairs_canonical`, `ordered_pairs_mixed`, `ordered_pairs_non_canonical` (the last three split `ordered_pairs` by how many of the two variants have `location_class` canonical: both, one, or neither; RR-03) | `analysis.threshold_sweep` |
