...
How to use: the Scrum Master (Allie Hodges) owns this page from Mon 2026-11-30 to Fri 2026-12-04. Nothing is submitted until every box in sections 1 and 2 is checked by the named verifier, who must not be the person who did the work.
...
| 5 | The repository contains the task backlog, issues, pull requests, reviews, decisions, tests, artifacts, and report materials. | | | Board, `docs/decisions/`, `tests/`, `results/`, `report/` | [ ] |
...
| 10 | Every member has documented an individual contribution and reflection. | | | `docs/reflections/` (4 files) | [ ] |
...
| Freeze `main`: only PRs labeled `rubric` or `bug` merge after Dec 1 | Announce in stand-up | | [ ] |
...
| Update `CHANGELOG` section in README or release notes | List commands, outputs, dataset snapshot | | [ ] |
...
| Complete `ai-use-log.md` and the disclosure paragraph in the report | Every member adds their entries | | [ ] |
| Submit individual reflections | `docs/reflections/<handle>.md` merged | | [ ] |
...
See `docs/PRESENTATION_PLAN.md` for the structure, speaker rotation, and backup plan.

## 5. Response-to-feedback log

| Date received | From | Feedback | Our response | Issue or PR # | Status |
|---|---|---|---|---|---|
| | | | | | |
...
| Member | Confirms sections 1 and 2 complete | Date |
|---|---|---|
| Leticia Aderhold | [ ] | |
| Jerad Dunne | [ ] | |
| Allie Hodges | [ ] | |
